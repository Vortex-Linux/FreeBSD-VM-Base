...
LATEST_IMAGE=$(lynx -dump -listonly -nonumbers https://download.freebsd.org/releases/VM-IMAGES | tail -n 2 | head -n 1 | xargs lynx -dump -listonly -nonumbers | grep amd64 | sed 's|$|Latest|' | xargs lynx -dump -listonly -nonumbers | sort -V | grep zfs | grep qcow | head -n 1)
